...
const Footer = () => (
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`} >
            ...
            <ul className='list-none mt-4'>
              {footerLink.links.map((link, index) => (
                <li key={link.name}
                 ...
                 hover:text-secondary cursor-pointer ${index !== footerLink.links.length - 1 ?
                      'mb-4' : 'mb:0 '}`}
                >
                  {link.name}
                </li>
    ...
    border-t-[1px] border-t-[#3F3e45]'>
      <p className='font-poppins font-normal text-center text-[18px] loading-[27px] text-white'>
      ...
      <div className='flex flex-row md:mt-0 mt-6'>
        {socialMedia.map((social, index) => (
          ...
          className={`w-[21px] h-[21px] object-contain cursor-pointer 
            ${index !== socialMedia.legnth - 1 ? 'mr-6' : 'mr-0' }`}/>
        ))}
      </div>
    </div>
  </section>
